      ...
      <div className="mx-auto max-w-7xl p-6 lg:px-8 grid md:grid-cols-3 gap-5">
        {data.map( (card, index) => {
          ...
          <div className="flex flex-col rounded-lg min-h-64 relative" key={card.titulo}>
            <img src={`./assets/image${index+1}.avif`} alt="" className=' object-cover' />
            <div className="flex flex-col justify-center absolute left-0 bottom-0 w-full p-3 text-white z-10">
              <h3 className='font-bold text-xl'>{card.titulo}</h3>
              <p className='text-sm'>{card.description}</p>
            </div>
            <div className='w-full absolute bottom-0 left z-0 bg-gradient-to-t from-[#000a25] from-0% min-h-[80%]'></div>
          </div> 
          )
        })}
      </div>
